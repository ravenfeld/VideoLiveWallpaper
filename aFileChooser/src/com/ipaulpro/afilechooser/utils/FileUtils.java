...
public class FileUtils {
	...
	/**
	 * File Filter that includes only files with the specified extensions to
	 * pass
	 * 
	 * @author Kiran Rao
	 * 
	...
	private static class FileExtensionFilter implements FileFilter {
		...
		public boolean accept(File file) {
			final String fileName = file.getName();
			boolean passesExtensionsFilter = mFilterIncludeExtensions.isEmpty() ? true : mFilterIncludeExtensions.contains(getExtension(Uri.fromFile(
					file).toString()));
			// Return files only (not directories) and skip hidden files
			return file.isFile() && !fileName.startsWith(HIDDEN_PREFIX) && passesExtensionsFilter;
		}
